...
/*
//-----------------------------------------------------------------------------
void	Sys::texture_Set( Texture* pTexture, int num, GLuint hdl_idTexture )
//-----------------------------------------------------------------------------
{
	gl_SetTexture( pTexture->target, pTexture->hdl_Texture, num, hdl_idTexture );
}

//-----------------------------------------------------------------------------
void	Sys::texture_Load( Texture* pTexture, int format, int type, void* pBuf )
//-----------------------------------------------------------------------------
{
	gl_LoadTexture( pTexture->target, pTexture->hdl_Texture, pTexture->format, pTexture->width, pTexture->height, format, type, pBuf );
}
//-----------------------------------------------------------------------------
Texture*	Sys::texture_Create( int target, int format, int width, int height, int min, int mag, int wrap )
//-----------------------------------------------------------------------------
{
	...
	gl_CreateTexture( &tex->hdl_Texture, target, format, width, height, min, mag, wrap );
...
//------------------------------------------------------------------------------
{
	...
	while (true)
	{
		...
		{
//			ret = true;
//			static	double	m_timerFrame = 0;
//			static	double	m_timerSec = 0;
//			LARGE_INTEGER m_frequency; 
			QueryPerformanceFrequency(&m_frequency);
