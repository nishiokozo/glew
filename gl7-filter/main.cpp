...
//-----------------------------------------------------------------------------
bool	FindValue( const char* name )
//-----------------------------------------------------------------------------
{
	int		n = 0;
	int		cnt = 0;
...
//			if ( win.m_pFuncPaint ) win.m_pFuncPaint();
			if ( win.m_pProc ) win.m_pProc->Paint();
			win.Swap();
		}
		break;
...
//------------------------------------------------------------------------------
{
	...
	shader_compile( m_hdlProgram, GL_VERTEX_SHADER, "f_ray.vs" ) ;
	shader_compile( m_hdlProgram, GL_FRAGMENT_SHADER, "f_ray.fs" ) ;
